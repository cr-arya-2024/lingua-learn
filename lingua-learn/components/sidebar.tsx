...
import { Home, Trophy, ShoppingBag, Target, Crown } from 'lucide-react';
import { cn } from '@/lib/utils';
const items = [{ label: 'Learn', href: '/learn', icon: Home }, { label: 'Leaderboard', href: '/leaderboard', icon: Trophy }, { label: 'Quests', href: '/quests', icon: Target }, { label: 'Shop', href: '/shop', icon: ShoppingBag }];
  ...
  return (
    <div className={cn('flex h-full lg:w-[256px] lg:fixed left-0 top-0 px-4 border-r-2 flex-col gap-y-2 bg-[#1a1a1a] border-neutral-800', className)}>
      ...
      <div className='flex flex-col gap-y-2 flex-1'>
        {items.map(item => (
          <Link key={item.href} href={item.href} className={cn('flex items-center gap-x-3 h-[52px] px-4 rounded-xl font-bold text-neutral-400 hover:bg-neutral-800 transition-all', pathname === item.href && 'bg-neutral-800 text-green-500 border-2 border-green-900')}>
